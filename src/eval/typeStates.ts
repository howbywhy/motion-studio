...
export async function runTypeStatesSheet(root: HTMLElement): Promise<TypeStatesReport> {
  ...
  for (const word of ["MADE", "FLAWED", "MADELEN"] as const) {
    for (const anchor of ["tl", "tc", "tr"] as TypeAnchor[]) {
      const state = headlinePage(word, anchor, 100);
      cell(capGrid, `${word}  ${ANCHOR_LABEL[anchor]}  ${cropCaption(state, W45, H45)}`, paintBleed(state, W45, H45));
    }
  }
  const descGrid = section(root, "Optical edge · descenders  Type / playing / quality / Sydney  ·  bottom may bleed");
  for (const word of ["Type", "playing", "quality", "Sydney"] as const) {
  ...
  const topCapitalsProtected =
    tlOver.t < 0.6 &&
    (["tl", "tc", "tr"] as TypeAnchor[]).every((anchor) =>
      ["MADE", "FLAWED", "MADELEN"].every((word) => inkOver(headlinePage(word, anchor, 100), W45, H45).t < 0.6),
    ) &&
    TYPE_ANCHORS.every((anchor) => inkOver(headlinePage("MADELEN", anchor, 100), W45, H45).t < 0.6) &&
    TYPE_ANCHORS.every((anchor) => inkOver(headlinePage("MADE", anchor, 100), W916, H916).t < 0.6);
  const bottomDescendersBleed =
    obviousCrop(bcOver.b, bcOver.inkH) &&
    obviousCrop(brOver.b, brOver.inkH) &&
    ["Type", "playing", "quality", "Sydney"].every((word) => inkOver(headlinePage(word, "bc", 100), W45, H45).b > 0.5);
  const headlineBleed =
