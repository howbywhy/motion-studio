...
/** Spatial sample bias for Loop pair variation. Identity when null. */
export interface BloomFieldBias {
  phase: number;
  originX: number;
  originY: number;
  coalesce: number;
...
let sampleBias: BloomFieldBias | null = null;

export function setBloomFieldSampleBias(bias: BloomFieldBias | null): void {
  sampleBias = bias;
}

export function getBloomFieldSampleBias(): BloomFieldBias | null {
  return sampleBias;
}

export function withBloomFieldSampleBias<T>(bias: BloomFieldBias | null, fn: () => T): T {
  const prev = sampleBias;
  sampleBias = bias;
  try {
    return fn();
  } finally {
...
export async function withBloomFieldSampleBiasAsync<T>(bias: BloomFieldBias | null, fn: () => Promise<T>): Promise<T> {
  const prev = sampleBias;
  sampleBias = bias;
  try {
    return await fn();
...
export function computeResolvedFields(
  ...
  let resolvePhase = (t % resolvePeriod) / resolvePeriod;
  if (bias) {
    resolvePhase = resolvePhase + bias.coalesce;
    resolvePhase = resolvePhase - Math.floor(resolvePhase);
  }
  ...
  return state.fields.map((field) => {
    ...
    let baseX = 0.5 + (field.sx * 2 - 1) * spread + (bias?.originX ?? 0);
    let baseY = 0.5 + (field.sy * 2 - 1) * spread + (bias?.originY ?? 0);
    ...
    const phase = field.phase + (bias?.phase ?? 0);
    let dx = driftFrac * width * Math.sin(t * field.freqX * 0.35 + phase);
    let dy = driftFrac * height * Math.cos(t * field.freqY * 0.35 + phase * 1.3);
    if (bias && bias.direction !== 0) {
      const c = Math.cos(bias.direction);
      const s = Math.sin(bias.direction);
      const rdx = dx * c - dy * s;
      const rdy = dx * s + dy * c;
      dx = rdx;
      dy = rdy;
    ...
    const localCycle = (t * field.localFreq + field.localPhase + (bias?.phase ?? 0) * 0.06) % 1;
    ...
    const rotation = field.rotation + t * field.rotationSpeed + (bias?.direction ?? 0);
