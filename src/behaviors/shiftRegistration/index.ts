// PRESERVED, UNREGISTERED: this was the second "01" implementation —
// large asymmetric regions with soft-registration seams, physically
// offsetting to glimpse B underneath. Judged too procedural: despite the
// asymmetric cuts and staggered timing it still read as "regions being
// moved over a photo" rather than the photo itself transforming. Replaced
// in the registry (src/behaviors/index.ts) by a rebuild of "Shift"
// (./shift/) built around fragmentation — every fragment blends A and B
// through its own transformation phase, rather than one image with
// windows cut into it. Kept here, intact and importable, in case that
// rebuild doesn't hold up either — re-add `shiftRegistrationBehavior` to
// the BEHAVIORS array in index.ts to restore it (and give it its own tab
// index, since "01" now belongs to the new Shift).
...
export const shiftRegistrationBehavior: MaskBehavior<ShiftBehaviorState> = {
  id: "shiftRegistration",
  name: "Shift (registration, legacy)",
  index: "01",
